...
const mockPath = path.join(__dirname, '../tests/infrastructure/vscode/settingsMock.ts');
...
let mockContent = `// THIS FILE IS AUTO-GENERATED
// DO NOT EDIT THIS FILE MANUALLY
...
for (const section of sections) {
  mockContent += `  public readonly ${section.name}: I${section.className} = {
${section.props.map(p => `    ${p.shortKey}: ${p.defaultValue}`).join(',\n')}
  };\n\n`;
}
